...
<div class="scroll-container">
    {#if marketData.length}
        <div class="scrolling-content">
            {#each marketData as item, i}
                {#if allowedIndices.includes(item["Index Name"].toLowerCase())}
                    <div class="dow-jones">
                        <span class="index-name company-name"
                            >{item?.["Index Name"]}</span
                        >
                        <span class="index-value current-price"
                            >{item["Current Value"]}</span
                        >
                        <span
                            style={`${item["Percentage Change"] > 0 ? "background-color: #6FD7A7" : "background-color: #FF6861"}`}
