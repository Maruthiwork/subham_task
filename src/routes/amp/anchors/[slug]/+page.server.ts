...
export async function load({ params, locals }: any) {
    ...
    try {
        ...
        await postErrorLogs({ err_message:`Request failed with status code ${error?.location}`,apiUrl:"", err_code:error?.location , page_url:`anchors/${slug}` , device:deviceType , environtment:EnvironmentConfig?.VITE_ENVIRONMENT },language);
